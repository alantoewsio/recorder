...
const PLUGIN_ID: &str = "systran.faster-whisper";
const DEFAULT_MODEL: &str = "small";
const DEFAULT_SAMPLE_RATE_HZ: u32 = 16_000;

pub const FASTER_WHISPER_PLUGIN_ID: &str = PLUGIN_ID;
...
pub fn descriptors() -> Vec<LocalAnalyzerPluginDescriptor> {
    vec![LocalAnalyzerPluginDescriptor {
        ...
        name: "Faster-Whisper",
        description:
            "Streams enhanced audio chunks to a local faster-whisper sidecar and emits transcript events.",
    }]
}

pub fn create_analyzer(id: &str) -> std::result::Result<Box<dyn AudioAnalyzer + Send>, String> {
    create_analyzer_with_config(id, FasterWhisperConfig::default())
}
...
pub fn create_analyzer_with_config(
    ...
    if id == PLUGIN_ID {
        Ok(Box::new(FasterWhisperAnalyzer::new(config)))
    } else {
...
impl DeviceMode {
    pub const ALL: [Self; 3] = [Self::Auto, Self::Cpu, Self::Cuda];
...
impl TranscriptMode {
    pub const ALL: [Self; 2] = [Self::FinalOnly, Self::PartialsAndFinals];

    pub fn label(self) -> &'static str {
        match self {
            Self::FinalOnly => "Final only",
            Self::PartialsAndFinals => "Partials and finals",
        }
    }
}
...
impl TaskMode {
    pub const ALL: [Self; 2] = [Self::Transcribe, Self::TranslateToEnglish];
    ...
    fn as_arg(self) -> &'static str {
        match self {
            Self::Transcribe => "transcribe",
            Self::TranslateToEnglish => "translate",
        }
    }
}
...
impl Default for FasterWhisperConfig {
    fn default() -> Self {
        let python = std::env::var("RECORDER_FASTER_WHISPER_PYTHON")
            .unwrap_or_else(|_| detect_project_python().unwrap_or_else(|| "python".into()));
        let worker_script = std::env::var("RECORDER_FASTER_WHISPER_WORKER")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
        ...
        let model = match std::env::var("RECORDER_FASTER_WHISPER_MODEL")
            .unwrap_or_else(|_| DEFAULT_MODEL.to_string())
            .to_ascii_lowercase()
            .as_str()
        {
            "tiny" => ModelChoice::Tiny,
            "base" => ModelChoice::Base,
            "medium" => ModelChoice::Medium,
            "large-v3" => ModelChoice::LargeV3,
            "distil-large-v3" => ModelChoice::DistilLargeV3,
            _ => ModelChoice::Small,
        };
        let chunk_seconds = std::env::var("RECORDER_FASTER_WHISPER_CHUNK_SECONDS")
            .ok()
            .and_then(|v| v.parse::<f32>().ok())
            .filter(|v| *v > 0.0)
            .unwrap_or(1.0);
        let silence_rms_threshold = std::env::var("RECORDER_FASTER_WHISPER_SILENCE_RMS")
            .ok()
            .and_then(|v| v.parse::<f32>().ok())
            .filter(|v| *v >= 0.0)
            .unwrap_or(0.005);
        ...
        Self {
            ...
            worker_script,
            chunk_seconds,
            pre_ready_buffer_seconds: 3.0,
            sample_rate_hz: DEFAULT_SAMPLE_RATE_HZ,
            stream_id: Some("default".to_string()),
            silence_rms_threshold,
            device_mode: DeviceMode::Auto,
            transcript_mode: TranscriptMode::PartialsAndFinals,
...
pub struct FasterWhisperAnalyzer {
    config: FasterWhisperConfig,
    worker: Option<FasterWhisperWorker>,
    worker_ready: bool,
...
impl FasterWhisperAnalyzer {
    ...
    fn ensure_worker(&mut self) -> Result<()> {
        ...
        match FasterWhisperWorker::start(&self.config) {
            Ok(worker) => {
                self.pending.push(MediaEvent::AttributeDetected {
                    tap: AudioTap::Processed,
                    start_frame: 0,
                    end_frame: 0,
                    key: "faster_whisper.status".to_string(),
                    value: format!(
            ...
            Err(e) => {
                self.pending.push(MediaEvent::AttributeDetected {
                    ...
                    start_frame: 0,
                    end_frame: 0,
                    key: "faster_whisper.error".to_string(),
                    value: e.clone(),
                    confidence: None,
                });
        ...
        while self.resampled_buffer.len() >= chunk_samples {
            ...
            if rms < self.config.silence_rms_threshold {
                self.pending.push(MediaEvent::AttributeDetected {
                    ...
                    start_frame,
                    end_frame,
                    key: "faster_whisper.segment".to_string(),
                    value: format!(
                ...
                let wav_path = write_segment_wav(
                    "recorder-faster-whisper",
                    self.segment_id,
                    self.config.sample_rate_hz,
                    &samples,
                )
                .map_err(|e| RecordingError::Plugin(format!("faster-whisper segment wav: {e}")))?;
    ...
    fn drain_worker_messages(&mut self) {
        ...
        for msg in messages {
            match msg {
                ...
                WorkerMessage::Stderr(line) => self.pending.push(MediaEvent::AttributeDetected {
                    tap: AudioTap::Processed,
                    start_frame: 0,
                    end_frame: 0,
                    key: "faster_whisper.stderr".to_string(),
                    value: line,
                    confidence: None,
                }),
            }
    ...
    fn handle_chunk_text(
        ...
        self.pending.push(MediaEvent::AttributeDetected {
            ...
            start_frame,
            end_frame,
            key: "faster_whisper.transcript".to_string(),
        ...
        if self.config.transcript_mode == TranscriptMode::PartialsAndFinals {
...
impl Default for FasterWhisperAnalyzer {
    fn default() -> Self {
        Self::new(FasterWhisperConfig::default())
    }
}

impl AudioAnalyzer for FasterWhisperAnalyzer {
    fn name(&self) -> &str {
        "faster-whisper"
    }
...
fn downmix_and_resample(input: &AudioBuffer, output_rate_hz: u32) -> Vec<f32> {
    ...
    if input.format.sample_rate_hz == output_rate_hz {
        return mono;
    }
...
impl FasterWhisperWorker {
    fn start(config: &FasterWhisperConfig) -> std::result::Result<Self, String> {
        let mut command = Command::new(&config.python);
        command
            .arg(&config.worker_script)
            .arg("--model")
            .arg(config.model.label())
            .arg("--device-mode")
            .arg(config.device_mode.as_arg())
            .arg("--task")
            .arg(config.task_mode.as_arg())
            .arg("--beam-size")
            .arg(config.beam_size.to_string())
            .arg("--vad-filter")
            .arg(if config.vad_filter { "true" } else { "false" })
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped());
        if let Some(cache_dir) = &config.model_cache_dir {
            command.arg("--cache-dir").arg(cache_dir);
        }

        let mut child = command.spawn().map_err(|e| {
            format!(
                "failed to start Faster-Whisper worker with {} {}: {e}",
                config.python,
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| "faster-whisper worker stdin unavailable".to_string())?;
        let stdout = child
            .stdout
            .take()
    ...
    fn send_segment(
        ...
        if let Ok(Some(status)) = self.child.try_wait() {
            return Err(RecordingError::Plugin(format!(
                "faster-whisper worker exited before segment {segment_id}: {status}"
            )));
        }
        ...
        .map_err(|e| RecordingError::Plugin(format!("faster-whisper worker stdin: {e}")))
    }
}
...
mod tests {
    ...
    #[test]
    fn partial_mode_emits_partial_updates() {
        let mut analyzer = FasterWhisperAnalyzer::default();
        analyzer.config.transcript_mode = TranscriptMode::PartialsAndFinals;
        analyzer.handle_chunk_text(0, 0, 24_000, "Hello there".to_string());

        assert!(analyzer.pending.iter().any(|event| matches!(
            event,
            MediaEvent::TranscriptPartial { text, .. } if text == "Hello there"
        )));
    }

    #[test]
    fn final_only_mode_suppresses_partial_updates() {
        let mut analyzer = FasterWhisperAnalyzer::default();
        analyzer.config.transcript_mode = TranscriptMode::FinalOnly;
        analyzer.handle_chunk_text(0, 0, 24_000, "Hello there".to_string());

        assert!(!analyzer
            .pending
            .iter()
            .any(|event| matches!(event, MediaEvent::TranscriptPartial { .. })));
    }

    #[test]
    fn aggregator_emits_final_when_sentence_completes() {
        let mut analyzer = FasterWhisperAnalyzer::default();
        ...
        let final_text = analyzer.pending.iter().find_map(|event| match event {
