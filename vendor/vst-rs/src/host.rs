...
pub struct PluginInstance {
    ...
    lib: Arc<Library>,
    info: Info,
    // Locally patched: was a `bool` that flipped to `true` on first `get_editor()` and was
    // never reset, preventing re-opening the editor window. We share the flag with the
    // returned `EditorInstance` so its `close()` can release the gate.
...
struct EditorInstance {
    params: Arc<PluginParametersInstance>,
    is_open: bool,
    // Locally patched: shared with the parent `PluginInstance` so `close()` can clear the gate.
    is_editor_active: Arc<AtomicBool>,
}
...
impl Editor for EditorInstance {
    ...
    fn close(&mut self) {
        ...
        self.is_open = false;
        // Locally patched: release the parent's gate so the editor can be opened again.
        self.is_editor_active.store(false, Ordering::Release);
    }
...
impl<T: Host> PluginLoader<T> {
    ...
    pub fn instance(&mut self) -> Result<PluginInstance, PluginLoadError> {
        ...
        // Many real-world VST 2.4 plugins (notably Cockos ReaPlugs) do not implement the
        // `effGetVstVersion` opcode and return 0 from the dispatcher's default branch. The
        // upstream check rejected them with `InvalidApiVersion`, even though every other
        // host (REAPER, Ableton, FL, etc.) accepts them. We log the reported value for
        // debugging and load anyway — the worst case is that a non-VST DLL slipped past
        // the `VSTPluginMain` export check, in which case the plugin's processor calls
        // will simply do nothing.
        //
        // (Locally patched in `vendor/vst-rs/`; not present in upstream vst-rs 0.4.0.)
        let api_ver = instance.dispatch(plugin::OpCode::GetApiVersion, 0, 0, ptr::null_mut(), 0.0);
        if api_ver < 2400 {
            trace!(
                "Plugin reports VST API version {} (<2400); loading anyway",
                api_ver
            );
        }
        Ok(instance)
...
impl Plugin for PluginInstance {
    ...
    fn get_editor(&mut self) -> Option<Box<dyn Editor>> {
        // Locally patched: gate is now an `Arc<AtomicBool>` shared with `EditorInstance::close()`,
        // so a host can repeatedly toggle the editor window across the lifetime of one
        // `PluginInstance` instead of being limited to a single open.
        if self
            .is_editor_active
        ...
        Some(Box::new(EditorInstance {
            ...
            is_open: false,
            is_editor_active: self.is_editor_active.clone(),
        }))
    }
