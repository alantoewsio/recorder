// Locally vendored copy of vst-rs 0.4.0 (see vendor/vst-rs/Cargo.toml). The crate is
// frozen upstream and accumulates new lints with each rust release; suppress them
// globally so they don't pollute downstream builds. Our own patches (host.rs version
// check) are explicitly marked in the source.
#![allow(warnings)]
