...
fn main() {
    ...
    // On Windows MSVC, prefer `CMAKE_GENERATOR` from the environment (set by
    // scripts/build-recorder-ui-vst-release.ps1) so the generator matches the installed CMake
    // (e.g. VS 18 2026 requires a new enough cmake) and the installed Visual Studio.

    // `cmake` caches the generator in `OUT_DIR/build`. If the toolchain or CMake version changes
    // (e.g. Visual Studio 17 2022 vs 18 2026), configure fails until that tree is removed.
    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());
    let cmake_build = out_dir.join("build");
    if cmake_build.is_dir() {
        let _ = fs::remove_dir_all(&cmake_build);
    }
